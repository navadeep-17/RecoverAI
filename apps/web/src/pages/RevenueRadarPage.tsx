...
export function RevenueRadarPage() {
  ...
  const metrics = [
    ...
    ['Agent-attributed recovered', formatMoney(metricsResponse.agentAttributedRecovered), 'Validated winning action correlation only', TrendingUp, 'text-sky-600'],
  ...
  return <div className="space-y-6"><div><p className="eyebrow">REVENUE RADAR</p><h1 className="page-title">Recovery operations, grounded in evidence</h1><p className="page-copy">AI proposes. Policy decides. Executor acts. Observer verifies.</p></div>
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">{metrics.map(([label, value, detail, Icon, tone]) => <section key={label as string} className="card"><div className="flex items-start justify-between"><p className="text-sm font-medium text-slate-600">{label as string}</p><Icon className={`h-5 w-5 ${tone as string}`} /></div><p className="mt-4 text-2xl font-bold tracking-tight text-slate-950">{value as string}</p><p className="mt-1 text-xs text-slate-500">{detail as string}</p></section>)}</div>
