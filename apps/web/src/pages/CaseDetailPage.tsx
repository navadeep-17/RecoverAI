...
export function CaseDetailPage({ caseId, navigate }: { caseId: string; navigate: (path: string) => void }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="card">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-4"><Fact label="Amount at risk" value={formatMoney(item.amountAtRisk, item.currency)} /><Fact label="Verified recovered" value={verifiedRecovered} /><Fact label="Recovery attribution" value={attribution} /><Fact label="Opened" value={formatDate(item.openedAt)} /></div>
    </section>
    <section className="card"><p className="eyebrow">RECOVERY DECISION</p><div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Fact label="Risk" value={`${riskLabel(item.riskType)} · ${formatMoney(item.amountAtRisk, item.currency)}`} /><VerifiedEvidence context={context} /><Fact label="AI proposal" value={plan ? `${plan.proposedActionType.replaceAll('_', ' ')} · ${Math.round(plan.confidence * 100)}%` : 'Not available'} /><Fact label="Policy" value={action ? `${action.policyDecision} · ${action.policyRationale || 'No persisted rationale.'}` : 'Not available'} /><Fact label="Execution" value={action ? `${action.status} · ${providerLabel(action.providerName)}` : 'No action yet'} /><Fact label="Observation" value={outcomes[0] ? outcomes[0].outcomeType : 'No authoritative outcome yet'} /><Fact label="Replan" value={(item.planVersions?.length || 0) > 1 ? `Plan v${plan?.version}` : 'No replan yet'} /><Fact label="Verified outcome" value={winner?.amountRecovered ? `${formatMoney(winner.amountRecovered, item.currency)} · ${attribution}` : 'No verified recovery yet'} /></div></section>
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      ...
      <Panel title="Actions">{item.actions?.length ? <ul className="space-y-3">{item.actions.map((entry) => <li key={entry.id} className="text-sm"><p className="font-semibold">{entry.actionType.replaceAll('_', ' ')}</p><p className="text-slate-500">{entry.status} · {providerLabel(entry.providerName)} · {formatDate(entry.executedAt || entry.createdAt)}</p></li>)}</ul> : <p className="text-sm text-slate-500">No actions have been authorized.</p>}</Panel>
      <Panel title="Outcomes">{outcomes.length ? <ul className="space-y-3">{outcomes.map((outcome) => <li key={outcome.id} className="text-sm"><p className="font-semibold">{outcome.outcomeType}</p><p className="text-slate-500">{outcome.amountRecovered ? formatMoney(outcome.amountRecovered, item.currency) : 'No monetary amount'} · {formatDate(outcome.observedAt)}</p><p className="text-slate-600">{winner?.id === outcome.id ? attribution : 'Non-monetary observation'}</p></li>)}</ul> : <p className="text-sm text-slate-500">No authoritative outcomes observed.</p>}</Panel>
...
function providerLabel(providerName?: string | null) { if (!providerName || providerName === 'UNKNOWN') return 'Internal / unclassified provider evidence'; if (/^SIMULATED_/i.test(providerName)) return 'Simulated provider'; if (providerName === 'RAZORPAY_TEST_MODE_PAYMENT_LINKS') return 'Razorpay Test Mode'; return 'Internal / unclassified provider evidence'; }
function evidenceFacts(context: Record<string, unknown>) { const string = (key: string) => typeof context[key] === 'string' ? context[key] as string : null; const retry = typeof context.retryAttemptNumber === 'number' ? String(context.retryAttemptNumber) : null; const network = string('cardNetwork'); const last4 = string('cardLast4'); return [['Failure', string('verifiedPaymentFailureCode')], ['Method', string('paymentMethod')], ['Card', network ? last4 ? `${network} •••• ${last4}` : network : null], ['Bank', string('bankName')], ['Retry attempt', retry], ['Gateway', string('gatewayErrorMessage')]].filter((fact): fact is [string, string] => Boolean(fact[1])); }
function VerifiedEvidence({ context }: { context: Record<string, unknown> }) { const facts = evidenceFacts(context); return <div><p className="text-xs font-medium uppercase tracking-wide text-slate-500">Verified evidence</p>{facts.length ? <dl className="mt-1 space-y-1 text-sm font-semibold text-slate-950">{facts.map(([label, value]) => <div key={label}><dt className="inline text-slate-500">{label}: </dt><dd className="inline">{value}</dd></div>)}</dl> : <p className="mt-1 font-semibold text-slate-950">Not available</p>}</div>; }
