...
function providerLabel(providerName?: string | null) { if (!providerName) return 'Internal / no external provider'; if (providerName.includes('SIMULATED')) return 'Simulated provider'; if (providerName === 'RAZORPAY_TEST_MODE_PAYMENT_LINKS') return 'Razorpay Test Mode'; return 'External provider'; }
function paymentEvidence(context: Record<string, unknown>) { const code = typeof context.verifiedPaymentFailureCode === 'string' ? context.verifiedPaymentFailureCode : null; const network = typeof context.cardNetwork === 'string' ? context.cardNetwork : null; const last4 = typeof context.cardLast4 === 'string' ? context.cardLast4 : null; return [code, network && last4 ? `${network} •••• ${last4}` : null].filter(Boolean).join(' · ') || 'Not available'; }
