...
async function main(): Promise<void> {
  ...
  try {
    ...
    const isSimulatedAction = (providerName: string | null) => Boolean(providerName && providerName.includes('SIMULATED'));
    const execution = { providerActions: actions.filter((action) => action.status === 'SUCCESS' && !isSimulatedAction(action.providerName)).length, simulatedActions: actions.filter((action) => action.status === 'SUCCESS' && isSimulatedAction(action.providerName)).length };
