...
import { RUNTIME_PROOF_LABEL, RUNTIME_PROOF_PROVIDER, RUNTIME_PROOF_SCENARIOS, assertRuntimeProofSafety, calculateRuntimeProofMetrics, classifyRuntimeExecutionEvidence, validateRuntimeProofScenarios } from '../packages/runtime-proof/src/index.js';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const successfulActions = actions.filter((action) => action.status === 'SUCCESS');
    const execution = {
      externalProviderActions: successfulActions.filter((action) => classifyRuntimeExecutionEvidence(action.providerName).category === 'externalProvider').length,
      simulatedActions: successfulActions.filter((action) => classifyRuntimeExecutionEvidence(action.providerName).category === 'simulated').length,
      internalActions: successfulActions.filter((action) => classifyRuntimeExecutionEvidence(action.providerName).category === 'internal').length,
      razorpayTestModeActions: successfulActions.filter((action) => classifyRuntimeExecutionEvidence(action.providerName).razorpayTestMode).length,
    };
