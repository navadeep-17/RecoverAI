...
export const EnvSchema = z
  .object({
    ...
    DATABASE_URL: z.string().default('postgresql://recoverai:recoverai_secret@localhost:5432/recoverai?schema=public'),
  ...
  .superRefine((env, ctx) => {
    const razorpayFields = [
      ...
      ['RAZORPAY_TEST_MERCHANT_ID', env.RAZORPAY_TEST_MERCHANT_ID],
      ['RAZORPAY_WEBHOOK_SECRET', env.RAZORPAY_WEBHOOK_SECRET],
    ] as const;
    const hasAnyRazorpayConfig = razorpayFields.some(([, value]) => Boolean(value));
    if (hasAnyRazorpayConfig) {
      for (const [field, value] of razorpayFields) {
        if (!value) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            path: [field],
            message: 'is required when Razorpay Test Mode is configured',
          });
        }
      }
    }
    if (env.RAZORPAY_KEY_ID && !env.RAZORPAY_KEY_ID.startsWith('rzp_test_')) {
      ctx.addIssue({
        ...
        message: 'must be a Razorpay Test Mode key (rzp_test_...); live and unrecognized keys are not supported',
      });
    }
    if (env.NODE_ENV === 'development' && env.AUTH_MODE !== 'dev_headers') {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['AUTH_MODE'],
        message: 'must be dev_headers for local development',
      });
    }
    if (env.NODE_ENV === 'production') {
      if (env.AUTH_MODE !== 'trusted_headers')
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['AUTH_MODE'],
          message: 'must be trusted_headers in production',
        });
      if (!env.AUTH_TRUST_SECRET)
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['AUTH_TRUST_SECRET'],
          message: 'is required for trusted production headers',
        });
      if (env.CORS_ORIGIN === '*' || env.CORS_ORIGIN.includes('localhost'))
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['CORS_ORIGIN'],
          message: 'must be an explicit non-localhost production origin',
        });
      if (env.SESSION_SECRET === 'development_secret_must_be_overridden_in_prod')
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['SESSION_SECRET'],
      ...
      if (env.AI_PROVIDER !== 'gemini')
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['AI_PROVIDER'],
          message: 'must explicitly be gemini in production; mock is development/test-only',
        });
    }
